/*
 * incApache_threads.c: implementazione dei thread per il web server del corso
 * di SET
 *
...
int no_response_threads[MAX_CONNECTIONS]; /* for each connection, how many
                                             response threads */

pthread_t thread_ids[MAX_THREADS];
int connection_no[MAX_THREADS];  /* connection_no[i] >= 0 means that i-th thread
                                    belongs to connection connection_no[i] */
pthread_t *to_join[MAX_THREADS]; /* for each thread, the pointer to the previous
                                    (response) thread, if any */

int no_free_threads =
    MAX_THREADS -
    2 * MAX_CONNECTIONS; /* each connection has one thread listening and one
                            reserved for replies */
struct response_params
    thread_params[MAX_THREADS -
                  MAX_CONNECTIONS]; /* params for the response threads (the
                                       first MAX_CONNECTIONS threads are
                                       waiting/parsing requests) */

pthread_mutex_t threads_mutex =
    PTHREAD_MUTEX_INITIALIZER; /* protects the access to thread-related data
                                  structures */
...
int find_unused_thread_idx(int conn_no) {
    ...
    if (no_response_threads[conn_no] > 0) { /* reserved thread already used, try
...
void *response_thread(void *vp) {
    ...
    send_response(client_sockets[connection_idx], thread_params[i].code,
                  thread_params[i].cookie, thread_params[i].is_http1_0,
                  (int)thread_no, thread_params[i].filename,
...
char *get_mime_type(char *filename) {
    ...
    debug("      ... get_mime_type(%s): printed filename on "
          "mime_request_stream\n",
          filename);
...
void send_resp_thread(int out_socket, int response_code, int cookie,
    ...
    struct response_params *params =
        thread_params + (new_thread_idx - MAX_CONNECTIONS);
    ...
    if (pthread_create(thread_ids + new_thread_idx, NULL, response_thread,
                       connection_no + new_thread_idx))
