...
void send_response(int client_fd, int response_code, int cookie,
    ...
    /*** Compute date of servicing current HTTP Request using a variant of
     * gmtime() ***/
    /*** TO BE DONE 8.0 START ***/
    ...
    case RESPONSE_CODE_OK:
        if (filename != NULL) {
            if (stat_p == NULL) {
                ...
                if (fd < 0) {
                    debug("send_response: cannot open file for reading (has "
                          "the file vanished?)");
                    response_code = RESPONSE_CODE_INTERNAL_ERROR;
                    goto int_err;
                }
            ...
            debug("      ... send_response(%3d,%s) : file opened, size=%lu, "
                  "mime=%s\n",
    ...
    case RESPONSE_CODE_NOT_FOUND:
        ...
        if ((fd = open(HTML_404, O_RDONLY)) >= 0) {
            ...
            /*** compute file_size, mime_type, and file_modification_time of
             * HTML_404 ***/
            /*** TO BE DONE 8.0 START ***/

            if (stat_p == NULL)
                stat_p = &stat_buffer;
            stat(HTML_404, stat_p);
            file_size = stat_p->st_size;
            file_modification_time = stat_p->st_mtime;
    ...
    if (file_size > 0 && mime_type != NULL) {
        sprintf(http_header + strlen(http_header),
                "Content-Length: %lu \r\nContent-Type: %s\r\nLast-Modified: ",
                (unsigned long)file_size, mime_type);

        /*** compute time_as_string, corresponding to file_modification_time, in
           GMT standard format; see gmtime and strftime ***/
        /*** TO BE DONE 8.0 START ***/
        gmtime_r(&file_modification_time, &file_modification_tm);
        strftime(time_as_string, MAX_TIME_STR, "%a, %d %b %Y %T GMT",
                 &file_modification_tm);
        /*** TO BE DONE 8.0 END ***/
...
#endif
#ifdef OptimizeTCP
    if ((header_sent = send_all(client_fd, http_header, header_size,
                                (fd >= 0 ? MSG_MORE : 0))) < header_size)
#else
    if ((header_sent = send_all(client_fd, http_header, header_size, 0)) <
        header_size)
#endif
    {
        if (header_sent == -1)
    ...
    if (fd >= 0) {
        ...
        while (offset < file_size) {
            bytes_sent = sendfile(client_fd, fd, &offset, file_size - offset);
            if (bytes_sent < 0) {
                // Se si verifica un errore, interrompi il ciclo e chiudi il
                // file
                perror("incApache: sendfile failed");
                break;
            }
        }
        close(fd);
...
#define MethodIsConditional(m) ((m) & METHOD_CONDITIONAL)
    ...
    while (getline(&http_request_line, &n, client_stream) >= 0) {
        ...
        debug(
            "   ... method_str=%s, filename=%s (0=%c), protocol=%s (len=%d)\n",
            method_str, filename, filename ? filename[0] : '?', protocol,
            (int)(protocol ? strlen(protocol) : 0));
        ...
        for (http_option_line = NULL, n = 0;
             getline(&http_option_line, &n, client_stream) >= 0 &&
            ...
            if (option_name != NULL) {
                if (strcmp(option_name, "Cookie") == 0) {
                    /*** parse the cookie in order to get the UserID and count
                     * the number of requests coming from this client ***/
                    /*** TO BE DONE 8.0 START ***/
                    option_val = strtok_r(NULL, " \t\r\n=", &strtokr_save);
                    if (option_val && strcmp(option_val, "UserID") == 0) {
                        option_val = strtok_r(NULL, " \t\r\n", &strtokr_save);
                        if (option_val)
                ...
                if (http_method == METHOD_GET) {
                    ...
                    /*** parse option line, recognize "If-Modified-Since"
                     *option,
        ...
        if (access(filename, R_OK) != 0 || stat(filename, stat_p) < 0) {
            ...
            if (MethodIsConditional(http_method)) {
                 ...
                 *** Use something like timegm() to convert from struct tm to
                 *time_t
                 ***/
                /*** TO BE DONE 8.0 START ***/
                time_t if_modified_since_t = my_timegm(&since_tm);
                if (stat_p->st_mtime <= if_modified_since_t) {
            ...
            case METHOD_HEAD:
                ...
                free(stat_p);
                SEND_RESPONSE(client_fd, RESPONSE_CODE_OK,
                              UIDcookie, /*** OK, without body ***/
#ifdef INCaPACHE_8_1
                              is_http1_0, connection_no, thread_idx,
#endif
                              filename, NULL);
            ...
            case METHOD_NOT_CHANGED:
                ...
                free(stat_p);
                SEND_RESPONSE(client_fd, RESPONSE_CODE_NOT_MODIFIED,
                              UIDcookie, /*** Not Modified, without body ***/
#ifdef INCaPACHE_8_1
                              is_http1_0, connection_no, thread_idx,
#endif
                              NULL, NULL);
