/*
 * incApache_main.c: implementazione del main per il web server del corso di
 * SETI
 *
...
void create_listening_socket(const char *const port_as_str) {
    ...
    if ((listen_fd = socket(server_addr->ai_family, server_addr->ai_socktype,
...
void run_file(const int *p_to_file, const int *p_from_file) {
    ...
    execlp("file", "file", "--no-buffer", "--brief", "--mime", "--files-from",
           "-", (char *)NULL);
    fprintf(stderr, "Cannot exec \"file\"\n");
    exit(-1);
}

void run_webserver(const char *const port_as_str, char *www_root,
                   const int *const p_to_file, const int *const p_from_file) {
    int i;
    ...
    for (i = 0; i < MAX_CONNECTIONS; i++) {
        ...
        /*** TO BE DONE 8.0 START ***/
        if (pthread_create(&thread_ids[i], NULL, client_connection_thread,
                           &connection_no[i]))
...
void check_uids() {
    ...
    if (geteuid()) {
        fprintf(stderr,
                "The effective UID should be zero (that is, the executable "
                "should be owned by root and have the SETUID flag on).\n");
        exit(EXIT_FAILURE);
    }
#endif /* #ifndef PRETEND_TO_BE_ROOT */
    if (getuid() == 0) {
        fprintf(stderr, "The real UID should be non-zero (that is, the "
                        "executable should be run by a non-root account).\n");
        exit(EXIT_FAILURE);
    }
}
